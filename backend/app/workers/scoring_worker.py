...
async def process_event(event_data: dict, broadcast_callback):
    async with async_session_maker() as db:
        ...
        anomaly_detector.add_training_sample(peer_group_id, feature_vector)
        ...
        if blended_score > 0.3:
            logger.info(f"Score for user {user_id}: ml={ml_score:.3f} rule={rule_score:.3f} blended={blended_score:.3f} (threshold={settings.ALERT_THRESHOLD})")
