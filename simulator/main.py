...
def create_population():
    ...
    n_normal = int(0.50 * total)        # Low risk (green) ~50%
    n_nightowl = int(0.20 * total)      # Medium risk (yellow)
    n_negligent = int(0.15 * total)     # Medium risk (yellow) 
    n_disgruntled = int(0.08 * total)   # High risk (red)
    n_compromised = int(0.07 * total)   # High risk (red)
    # Ensure total matches
    remainder = total - n_normal - n_nightowl - n_negligent - n_disgruntled - n_compromised
    n_normal += remainder
